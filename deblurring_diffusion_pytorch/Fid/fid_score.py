...
from deblurring_diffusion_pytorch.Fid import InceptionV3
...
def compute_statistics_of_samples(
...
def calculate_fid_given_samples(
    samples, batch_size=50, device="cuda:0", dims=2048, num_workers=1
):
    ...
    m1, s1 = compute_statistics_of_samples(
        samples[0], model, batch_size, dims, device, num_workers
    )
    m2, s2 = compute_statistics_of_samples(
        samples[1], model, batch_size, dims, device, num_workers
    )
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)
